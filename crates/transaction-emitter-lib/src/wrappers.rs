...
pub async fn emit_transactions_with_cluster(
    ...
    let mode_params = if args.mempool_backlog > 0 {
        assert!(
            args.fixed_tps == 0,
            "Cannot set both mempool-backlog and fixed-tps"
        );
        EmitJobMode::MaxLoad {
        ...
        assert!(
            args.fixed_tps > 0,
            "Need to set either mempool-backlog or fixed-tps"
        );
        EmitJobMode::FixedTps {
            tps: args.fixed_tps,
        }
    };
